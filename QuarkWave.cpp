...
#define WF_MODE 0  // 0 = off (envelope-only LEDs, current behavior) \
                   // 1 = capture 8-bit level (no FFT) \
                   // 2 = capture + leave room for FFT (still offloaded from audioTick)
...
void ledsVU() {
  static float hold = 0;
  float level = 0;
  for (int i = 0; i < NUM_VOICES; i++) level = maxf(level, V[i].eg.env);
  hold = maxf(hold * 0.96f, level);
  ledsClear();
  int bars = (int)roundf(level * 11);
  int peak = (int)roundf(hold * 11);
  for (int c = 0; c <= bars; c++) ledsSet(c, 4, true);
  if (peak >= 0 && peak < 12) ledsSet(peak, 4, true);
  drawTopPips();
  drawStatusOverlays();
  ledsRender();
}
...
void ledsScope() {
  ...
  float y = 0.0f;
  for (int i = 0; i < NUM_VOICES; ++i) y = maxf(y, V[i].eg.env);

  int row = 1 + (int)roundf((1.0f - y) * 6.0f);
...
void ledsWater() {
  ...
  for (int c = 0; c < 12; ++c) ledBuf[0][c] = 0;

  for (uint8_t c = 0; c < WF_COLS; ++c) {
    float thr = WF_GAIN * maxf(0.02f, wf_thresh[c]);
    if (wf_bins[c] > thr) ledBuf[0][c] = 1;
  }
...
void processUARTCommand(const char* cmd) {
  ...
  Serial.print(F("[processUARTCommand] [UART] cmd: "));
  Serial.println(t);
...
void setupPICO() {
  ...
  PICO_MIDI.setHandleProgramChange([](byte ch, byte program) {
      switch(program) {
          case 0:   // Reset to defaults
              resetToDefaults();
              break;
              
          case 100: // External tempo source
              tempoSrc = 1;
              Serial.println("[PC] Tempo source: External");
              break;
              
          case 101: // Internal tempo source
              tempoSrc = 0;
              Serial.println("[PC] Tempo source: Internal");
              break;
              
          case 102: // Delay sync on
              dlySync = true;
              Serial.println("[PC] Delay sync: ON");
              break;
              
          case 103: // Delay sync off
              dlySync = false;
              Serial.println("[PC] Delay sync: OFF");
              break;
              
          case 110: // LFO2 Sine wave
              lfo2Wave = LFO_SINE;
              Serial.println("[PC] LFO2 wave: Sine");
              break;
              
          case 111: // LFO2 Triangle wave
              lfo2Wave = LFO_TRI;
              Serial.println("[PC] LFO2 wave: Triangle");
              break;
              
          case 112: // LFO2 Square wave
              lfo2Wave = LFO_SQR;
              Serial.println("[PC] LFO2 wave: Square");
              break;
              
          default:
              Serial.print("[PC] Unknown program: ");
              Serial.println(program);
              break;
      }
  });
  PICO_MIDI.setHandleControlChange([](byte ch, byte cc, byte val) {
      handleCC(cc, val);
  });
  PICO_MIDI.setHandleNoteOn([](byte ch, byte note, byte vel) {
      synthNoteOn(note, vel);
  });
  PICO_MIDI.setHandleNoteOff([](byte ch, byte note, byte vel) {
      synthNoteOff(note);
  });
  PICO_MIDI.setHandlePitchBend([](byte ch, int bend) {
      setPitchBend((int16_t)(bend + 8192));
  });
  PICO_MIDI.setHandleSystemExclusive([](byte* sysex, unsigned size) {
      // Check for QuarkWave messages (F0 7D 00 ...)
      if (size >= 4 && sysex[0] == 0xF0 && sysex[1] == 0x7D && sysex[2] == 0x00) {
          uint8_t command = sysex[3];
          
          switch(command) {
              case 0x01: // Set BPM
                  if (size >= 7) {
                      uint16_t bpm = sysex[4] | (sysex[5] << 7);
                      if (bpm >= 20 && bpm <= 300) {
                          bpmInt = (float)bpm;
                          Serial.print("[SysEx] BPM set to ");
                          Serial.println(bpm);
                      }
                  }
                  break;
                  
              case 0x02: // Modulation routing
                  if (size >= 7) {
                      uint8_t routingType = sysex[4];
                      float amount = (sysex[5] & 0x7F) / 127.0f;
                      
                      if (routingType == 0) {
                          lfoToMorph = amount;
                          Serial.print("[SysEx] LFO->Morph = ");
                          Serial.println(amount);
                      } else if (routingType == 1) {
                          lfoToAmp = amount;
                          Serial.print("[SysEx] LFO->Amp = ");
                          Serial.println(amount);
                      }
                  }
                  break;
                  
              case 0x03: // Chorus depth
                  if (size >= 6) {
                      chDepth = (float)(sysex[4] & 0x7F);
                      Serial.print("[SysEx] Chorus depth = ");
                      Serial.println(chDepth);
                  }
                  break;
                  
              default:
                  Serial.print("[SysEx] Unknown command: ");
