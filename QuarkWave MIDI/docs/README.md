...
The connected controller identified over USB on 2026-09-23 is a **Pico W (RP2040, 2 MB flash)**. Its deployed build uses a 1 MB LittleFS region to match the existing patch storage. The Uno identified as **Arduino UNO R4 WiFi**. Both uploads verified; the last live panel check showed **Pico: Connected** and **Uno: Connected to Pico · sound loaded** with the existing “Test Pluck” user patch. An automated browser pass exercised 50 control paths and 41 pointer keys, plus all 12 computer keys at three velocities. The Pico reported changes and the Uno handshake was initially connected, but the Uno does not acknowledge each sound command. A Pico return-message parser fix resolved the reproduced explicit-sync failure in repeated two-board tests. A temporary Uno diagnostic confirmed browser Note On, envelope movement, and a finite pre-DAC signal on one patch; audible output and per-control sound response remain unverified. The latest Uno LED-status source also runs in the [USB-audio experiment](../experiments/usb-audio/README.md): the Pico link stayed connected during recordings and the owner confirmed the matrix responds to notes, but each new status pixel has not been individually accepted. See the [hardware test log](hardware-test-log.md).
