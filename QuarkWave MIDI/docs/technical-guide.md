...
| Network MIDI | Pico AppleMIDI instance named `QuarkWave` on UDP port `5004`; the Pico advertises `_apple-midi._udp` by mDNS. Uno `USE_RTP_MIDI=0`, `USE_WIFI_STACK=0`. A wireless controller requires the Pico. |
...
The Uno's 12×8 matrix has three Pico-selected modes: **Status** shows four voice-envelope bars, **VU** shows a bar and peak from the synthesized output level, and **Scope** plots an automatically scaled trace of output samples. The top row continues to show the Pico handshake, UART receive, forwarded wireless sound activity, note, and heartbeat indicators. The old Uno Wi-Fi and RTP-session pixels remain dark in this build; session status exists on the Pico. A bottom-right pixel indicates an LED update interval over 200 ms. Boot text scrolls, and view changes scroll the full **Viz: Status**, **Viz: VU**, or **Viz: Scope** label. The [LED display guide](led-display-guide.md) has a live photograph and labeled indicator table. These meters derive from firmware samples before the DAC; they are not calibrated measurements of the unbuilt audio output stage. [Uno matrix code](../QuarkWave_MIDI/QuarkWave_MIDI.ino).
...
The **Pico** label reflects the browser WebSocket connection; **RTP-MIDI** shows whether a network controller has joined the Pico session. The **Uno** label reflects replies to Pico readiness requests on the return MIDI wire; this establishes a responding firmware link, but does not prove audio output. `standalone sound` means a DIN sound message or a Pico-forwarded RTP sound message was received since the last Pico patch transfer. Clock and transport messages alone do not mark standalone sound. [Browser status](../QuarkWave_UI_MIDI/QuarkWave_UI.h), [Pico link handling](../QuarkWave_UI_MIDI/QuarkWave_UI_MIDI.ino).

## Pico ↔ Uno MIDI contract

The Pico sends its browser and patch messages on channel 1 of hardware `Serial1`; RTP messages retain their incoming channel while crossing the same UART. The Uno's `PICO_MIDI` receiver listens omni. Its separate software-serial DIN receiver also listens omni. Pico UART and DIN accept Note On/Off, CC, bend, Clock, and Start/Continue/Stop; the Uno keeps separate clock histories for DIN and the Pico-forwarded RTP stream, with DIN priority. The Pico filters RTP Program Changes and SysEx to the sound-related subset below before forwarding them. [Pico gateway](../QuarkWave_UI_MIDI/QuarkWave_UI_MIDI.ino), [Uno receiver handlers](../QuarkWave_MIDI/QuarkWave_MIDI.ino).
