...
The **Pico** label reflects the browser WebSocket connection; **RTP-MIDI** shows whether a network controller has joined the Pico session. **Uno: Connected to Pico** means the Uno has answered recent readiness requests on the return MIDI wire. **Sound loaded** additionally means it acknowledged the Pico's selected sound settings. These labels establish a responding firmware link, but do not prove audio output. **Standalone sound** means a DIN sound message or a Pico-forwarded RTP sound message was received since the last Pico sound transfer. Clock and transport messages alone do not mark standalone sound. [Browser status](../QuarkWave_UI_MIDI/QuarkWave_UI.h), [Pico link handling](../QuarkWave_UI_MIDI/QuarkWave_UI_MIDI.ino).
