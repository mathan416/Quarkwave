...
| Optional direct USB MIDI and audio | The [composite Uno experiment](../experiments/usb-audio/README.md) adds a USB MIDI 1.0 input beside the 22,050 Hz mono USB audio input and native USB serial. It keeps the D0/D1 Pico UART active. Its USB product and MIDI interface strings are **QuarkWave USB MIDI**; a stable experiment-specific serial suffix lets macOS distinguish it from its cached Uno entry. A macOS USB MIDI note and simultaneous audio recording passed under that name; DAW tests remain open. |
...
The browser presents one live instrument through **Perform**, **Shape**, **Explore**, **All controls**, and **Help** tabs. Help contains nine full guides under **User Guide** and **Technical details**. The Pico serves each guide and its images when selected, keeping the main panel compact. `tools/build-onboard-help.cjs` renders the Markdown sources into compressed program-flash assets in `QuarkWave_Help.h`; routes under `/help/` deliver them to the browser. The PDFs stay in the repository. Its topic choice is remembered in browser `localStorage` (`quarkwave-help-page`) and is not part of a patch. The keyboard and patch strip remain mounted as tabs change, and the selected main tab is saved only in browser `localStorage` (`quarkwave-view`). Perform's quick controls mirror the corresponding detailed controls. All controls moves the existing Shape and Explore cards into one scrollable panel and moves them back when another tab is selected; it does not clone inputs or create duplicate control IDs. Tab or Help-page selection itself sends no WebSocket message. [Embedded panel](../QuarkWave_UI_MIDI/QuarkWave_UI.h).
...
The Pico sends its browser and patch messages on channel 1 of hardware `Serial1`. RTP sound CCs other than sustain, pitch bend, and Program Changes retain their incoming channel while crossing the same UART; Clock and transport are channel-independent. RTP notes and sustain use the Pico-only source-tagged SysEx commands `15` and `16` below so the Uno can distinguish them from browser and DIN input. The Uno's `PICO_MIDI` receiver listens omni; its separate software-serial DIN receiver also listens omni. Both accept sound controls, Clock, and Start/Continue/Stop. The optional composite build adds a USB MIDI receiver on all channels. The Uno keeps separate clock histories for DIN, direct USB, and the Pico-forwarded RTP stream, with **DIN → USB → RTP** priority. The Pico filters RTP Program Changes and SysEx to the sound-related subset below before forwarding them. [Pico gateway](../QuarkWave_UI_MIDI/QuarkWave_UI_MIDI.ino), [Uno receiver handlers](../QuarkWave_MIDI/QuarkWave_MIDI.ino), [USB receiver](../experiments/usb-audio/UsbMidiInput.h).

The Pico admits one RTP-MIDI controller session at a time. It tracks held wireless pitches and sustain across the controller's MIDI channels, then releases both when AppleMIDI reports a disconnect. The Uno tracks note and sustain ownership separately for browser/Pico, DIN, forwarded RTP, and optional direct USB input. A wireless disconnect follows the normal release envelope without cutting off the same pitch or pedal held by another controller. In the composite build, a USB disconnect similarly releases only USB-owned notes and pedal state if the Uno remains powered. A DAW stopping MIDI output while keeping its USB port open is not a disconnect. A silent network loss is detected only when the AppleMIDI session times out. [Pico release path](../QuarkWave_UI_MIDI/QuarkWave_UI_MIDI.ino), [Uno ownership](../QuarkWave_MIDI/QuarkWave_MIDI.ino), [USB release path](../experiments/usb-audio/UsbMidiInput.h).
...
Commands `11` and `13` travel **Uno → Pico**. In their one-byte flags, bit 0 means a note, CC, pitch bend, or accepted sound command arrived on DIN or optional direct USB, or the Pico sent marker `14` for forwarded RTP sound activity, since boot or the last completed Pico patch; bit 1 means a Pico patch completion marker has been received. MIDI clock alone does not set bit 0. These flags describe received messages, not the electrical presence of a controller or the exact sound state. The Pico considers the Uno disconnected if no reply arrives for about 3.5 seconds. RTP sound activity received before a clean Uno handshake blocks automatic patch sync; an explicit sync clears that gateway activity state. The Pico parses the fixed six-byte Uno status and completion replies directly from `Serial1`, checking the manufacturer, command, flags, and boundaries. The generic MIDI receive parser had intermittently reported valid raw replies as two-byte SysEx messages and discarded the handshake. [Pico handshake](../QuarkWave_UI_MIDI/QuarkWave_UI_MIDI.ino), [Uno handshake](../QuarkWave_MIDI/QuarkWave_MIDI.ino).

### External MIDI Clock and transport

DIN, optional direct USB MIDI, and Pico-forwarded RTP-MIDI each maintain an independent 24-pulse-per-quarter-note clock history in the Uno. It accepts a measured 40–240 BPM and chooses a recent source in **DIN → USB → RTP** order. After two seconds without pulses from the selected source, it switches to another recent source or keeps the last BPM. Before the first valid stream, external BPM is 120. Clock alone does not select external tempo or mark standalone activity. Program Change 100 selects external tempo; 101 selects internal tempo. USB and network clock timing are hardware untested. A USB MIDI note and simultaneous audio recording passed, but that test did not send clock. [Clock receiver and selection](../QuarkWave_MIDI/QuarkWave_MIDI.ino), [USB packet handler](../experiments/usb-audio/UsbMidiInput.h), [Pico transport forwarding](../QuarkWave_UI_MIDI/QuarkWave_UI_MIDI.ino).
...
If the Uno has received a standalone DIN or optional USB note/control, or the Pico has received RTP sound activity, the Pico does **not** reset it automatically. The browser reports **Uno: Connected to Pico · standalone sound**. A user may select **Sync Pico patch to Uno**, acknowledge the held-note/current-sound warning, and explicitly replace that sound. Loading a patch or committed sound through the browser is also an explicit sound change. The Uno itself keeps no saved patch files: without the Pico it boots with built-in defaults and remains playable from DIN or the optional composite USB MIDI input; wireless MIDI requires the Pico. [Browser action](../QuarkWave_UI_MIDI/QuarkWave_UI.h), [Uno standalone inputs](../QuarkWave_MIDI/QuarkWave_MIDI.ino), [USB input](../experiments/usb-audio/UsbMidiInput.h).
...
The **optional composite USB audio/MIDI build** adds USB servicing and ring-buffer delivery to that workload. Its clean steady interval was 22,048.1 DAC writes/s with no new USB audio errors; its four-voice, all-effects interval fell to 21,321.6 writes/s with zero-filled USB audio underruns and scheduler slips. The USB host's 22,050-sample/s capture length alone is therefore not proof that the synth generated every sample. The [USB regression record](hardware-test-log.md#usb-midi-audio-regression-suite--2026-09-23) separates the measurements. The composite build needs more optimization or a documented performance limit before that worst-case patch can be called real-time safe.
